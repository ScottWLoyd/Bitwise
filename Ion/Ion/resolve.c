    ...
    if (decl->var.expr)
    {
        ...
        if (type && result.type != type)
        {
            if (type->kind == TYPE_ARRAY &&
                ...
                type->array.elem == result.type->array.elem &&
                !type->array.size)
            {
...
Val eval_binary_op(TokenKind op, Type* type, Val left, Val right)
{
    ...
    else
    {
        ...
        unsigned long long x = left_operand.val.ull;
        unsigned long long y = right_operand.val.ull;
        ...
        result_operand = operand_const(type_longlong, (Val) { .ull = r });
    }
...
Operand resolve_expr_ternary(Expr* expr, Type* expected_type)
{
    ...
    Operand then_expr = ptr_decay(resolve_expected_expr(expr->ternary.if_true, expected_type));
    Operand else_expr = ptr_decay(resolve_expected_expr(expr->ternary.if_false, expected_type));
...
Operand resolve_expr_cast(Expr* expr)
{
    ...
    {
        if (result.type->kind != TYPE_PTR && result.type-> != TYPE_INT)
        {
