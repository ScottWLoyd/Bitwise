...
bool type_ranks[NUM_TYPE_KINDS] = {
    [TYPE_CHAR] = 1,
    [TYPE_UCHAR] = 1,
    [TYPE_SCHAR] = 1,
    [TYPE_SHORT] = 2,
    ...
    if (decl->var.expr)
    {
        ...
        if (type)
        {
            if (type->kind == TYPE_ARRAY &&
                ...
                type->array.elem == operand.type->array.elem &&
                !type->array.size)
            {
...
Type* resolve_decl_const(Decl* decl, Val* val)
{
    ...
    if (!result.is_const)
    {
        fatal_error(decl->pos, "Const initializer for const is not const");
    }
    *val = result.val;
    return result.type;
}
...
unsigned long long eval_unary_op_ull(TokenKind op, unsigned long long val)
{
    ...
    case TOKEN_ADD:
        return +val;
    case TOKEN_SUB:
        return 0ull - val; // work around MSVC unary minus warning
    case TOKEN_NEG:
        return ~val;
    case TOKEN_NOT:
        return !val;
...
Val eval_binary_op(TokenKind op, Type* type, Val left, Val right)
{
    ...
    if (is_signed_type(type))
    {
        ...
        result_operand = operand_const(type_llong, 
            (Val) { .ll = eval_binary_op_ll(op, left_operand.val.ll, right_operand.val.ll) });
    }
    else
    {
        convert_operand(&left_operand, type_ullong);
        convert_operand(&right_operand, type_ullong);
        result_operand = operand_const(type_ullong, 
            (Val) { .ull = eval_binary_op_ull(op, left_operand.val.ull, right_operand.val.ull) });
    }
...
Operand resolve_expr_unary(Expr* expr)
{
    ...
    switch (expr->unary.op)
    {
        case TOKEN_MUL:
            ...
            {
                fatal_error(expr->pos, "Cannot deref a non-ptr type");
            }
...
Operand resolve_expr_ternary(Expr* expr, Type* expected_type)
{
	...
	Operand left = ptr_decay(resolve_expected_expr(expr->ternary.if_true, expected_type));
	Operand right = ptr_decay(resolve_expected_expr(expr->ternary.if_false, expected_type));
	if (is_arithmetic_type(left.type) && is_arithmetic_type(right.type))
	{
        unify_arithmetic_operands(&left, &right);
    ...
    {
		fatal_error(expr->pos, "Left and right operands of ternary expression must have arithmetic typeps or identical types");
    }
}
