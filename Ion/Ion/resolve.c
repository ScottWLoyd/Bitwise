...
Operand resolve_expr_ternary(Expr* expr, Type* expected_type)
{
	...
	Operand left = resolve_expected_expr_rvalue(expr->ternary.if_true, expected_type);
	Operand right = resolve_expected_expr_rvalue(expr->ternary.if_false, expected_type);
	if (is_arithmetic_type(left.type) && is_arithmetic_type(right.type))
	{
        unify_arithmetic_operands(&left, &right);
...
Operand resolve_expr_int(Expr* expr)
{
    ...
    if (expr->int_lit.mod == MOD_NONE)
    {
        ...
        switch (expr->int_lit.suffix)
        {
            case SUFFIX_NONE:
                type = type_int;
                // TODO: MAX constants should be sourced from the backend target table, not the host compiler's header files
                if (val > INT_MAX)
                {
                    type = type_long;
                    if (val > LONG_MAX)
