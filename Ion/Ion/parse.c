...
bool is_cmp_op()
{
	return is_token('<') || is_token('>') || is_token(TOKEN_EQ) || 
		is_token(TOKEN_NOTEQ) || is_token(TOKEN_GTEQ) || is_token(TOKEN_LTEQ);
}
...
Expr* parse_expr_ternary()
{
	...
	if (match_token('?'))
	{
		Expr* if_true = parse_expr_ternary();
		expect_token(':');
		Expr* if_false = parse_expr_ternary();
		expr = expr_ternary(expr, if_true, if_false);
	}
	return expr;
}
